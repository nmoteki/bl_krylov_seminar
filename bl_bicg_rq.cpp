...
MatrixXcd bl_bicg_rq(const MatrixXcd& A, const MatrixXcd& B, const double& tol, const int& itermax)
{
  ...
  for(int k= 0; k < itermax; ++k){
      ...
      MatrixXcd alpha= (V_hat.adjoint()*W).partialPivLu().solve(Q_hat.adjoint()*Q);
      MatrixXcd alpha_hat= (V.adjoint()*W_hat).partialPivLu().solve(Q.adjoint()*Q_hat);
      ...
      MatrixXcd beta= (Q_hat.adjoint()*Q).partialPivLu().solve(S_hat.adjoint()*Qnew_hat.adjoint()*Qnew);
      MatrixXcd beta_hat= (Q.adjoint()*Q_hat).partialPivLu().solve(S.adjoint()*Qnew.adjoint()*Qnew_hat);
